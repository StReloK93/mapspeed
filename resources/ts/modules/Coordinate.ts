...
export async function initWialon(sid, UIData) {
    var session = wialon.core.Session.getInstance()
    
    const { map } = new Leaflet(sid)

    const groups:any = await initReports(session, map)
...
export async function initReports(session, map) {
    function update_renderer() {
        var sess = wialon.core.Session.getInstance()
        var renderer = sess.getRenderer()
        if (Tlayer && Tlayer.setUrl) Tlayer.setUrl(getTiles(sess, renderer))
    }
    ...
    return await new Promise(function (resolve, reject) {
        ...
        session.updateDataFlags([
            ...
            resolve(units)
            executeReport(map, units[0].getId())
        })
    })
}


export function executeReport(map, group_id) { // execute selected report
    var sess = wialon.core.Session.getInstance()
    var renderer = sess.getRenderer()
    var report = sess.getItems("avl_resource")[0]
    var template = report.getReports()

    report.execReport(template[1], group_id, 0, { "from": from, "to": to, "flags": wialon.item.MReport.intervalFlag.absolute },
        (code, layer) => {
            if (!layer) return
...
export function getTiles(sess, renderer) {
    return sess.getBaseUrl() + "/adfurl" + renderer.getVersion() + "/avl_render/{x}_{y}_{z}/" + sess.getId() + ".png"
}
...
//     public executeReport(group_id) { // execute selected report
